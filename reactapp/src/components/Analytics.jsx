...
export default function EnhancedAnalyticsDashboard() {
  ...
  const activityData = useMemo(() => {
    ...
    console.log('Max orders in a day:', maxOrders);
    console.log('Sample order counts:', orderCounts.slice(0, 10));
    ...
    activityMap.forEach(dayData => {
      ...
      if (maxOrders > 0) {
        if (dayData.orders === 0) {
          dayData.level = 0;
        } else if (dayData.orders === 1) {
          dayData.level = 1;
        } else if (dayData.orders <= 3) {
          dayData.level = 2;
        } else if (dayData.orders <= 5) {
          dayData.level = 3;
        } else {
          dayData.level = 4;
        }
      ...
      if (dayData.orders > 0) {
        console.log(`Date: ${dayData.date}, Orders: ${dayData.orders}, Level: ${dayData.level}`);
      }
    });
  ...
  const stats = useMemo(() => {
    ...
    const lowStockProducts = products
      ...
      .sort((a, b) => (a.stockQuantity || a.stock || p.quantity || 0) - (b.stockQuantity || b.stock || b.quantity || 0))
      .slice(0, 5);
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div>
              ...
              <p className="text-gray-600 text-lg">Real-time insights from your business data</p>
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-3">
              ...
              Revenue Trend {dateFilter === 'today' ? '(Today)' : `(${dateFilter === 'custom' ? 'Custom Range' : dateFilter.charAt(0).toUpperCase() + dateFilter.slice(1)})`}
            </h3>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="space-y-4">
              {stats.lowStockProducts.map((product) => {
                ...
                return (
                  <div key={product.id} className="flex justify-between items-center p-4 bg-gradient-to-r from-red-50 to-pink-50 rounded-xl border border-red-200">
                    <div>
